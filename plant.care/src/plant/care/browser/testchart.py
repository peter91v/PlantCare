...
class TestChartView(BrowserView, ConfigFunctions):
...
#{'Sensors': 'Sensor1', 'count': '', 'dateVon': '', 'dateBis': '', 'timevon': '', 'timeBis': ''}
        ...
        return super(TestChartView, self).__call__(*args, **kwargs)
    ...
    def getdata(self):
        ...
        s = 0
        #sensors = sensors[0]
        temperatures = ['temp_trocken',
                        'temp_nass']
        for sen in temperatures:
            ...
            hum = []
            messdata = {}
            with open('/home/pi/Plant_Care/Zope/src/plant.care/src/plant/care/browser/utils/py/' + sen + '.json', 'r') as file:
                messdata = json.load(file)
                file.close()

            for i in range(0, 90, 1):
                if (str(i) not in messdata.keys()):
                    hum.append(0)
                    time.append(str(i))
                else:
                    hum.append(messdata[str(i)])
                    time.append(str(i))
